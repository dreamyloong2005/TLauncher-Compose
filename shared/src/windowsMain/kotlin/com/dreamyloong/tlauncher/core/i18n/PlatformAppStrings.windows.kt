...
actual val AppStrings.settingsManageStorageGranted: String
    get() = when (language) {
        SupportedLanguage.ZH_CN -> "Windows 端可直接访问启动器工作目录。"
        SupportedLanguage.EN_US -> "Windows can access the launcher workspace directly."
    }
...
actual val AppStrings.settingsManageStorageActionGrant: String
    get() = when (language) {
        SupportedLanguage.ZH_CN -> "打开储存设置"
        SupportedLanguage.EN_US -> "Open Storage Settings"
    }
...
actual fun AppStrings.homeStoragePermissionWarningSubtitle(templateName: String): String {
    return when (language) {
        SupportedLanguage.ZH_CN -> "$templateName 在 Windows 端不需要额外储存授权。"
        SupportedLanguage.EN_US -> "$templateName does not need extra storage authorization on Windows."
    }
}
...
actual fun AppStrings.settingsLauncherIconTitle(): String {
    return when (language) {
        SupportedLanguage.ZH_CN -> "窗口图标"
        SupportedLanguage.EN_US -> "Window Icon"
    }
}
...
actual fun AppStrings.settingsLauncherIconSubtitle(
    ...
    return when (language) {
        SupportedLanguage.ZH_CN -> "当前窗口图标：$currentIconName。跟随主题时会使用：$themeIconName。"
        SupportedLanguage.EN_US -> "Current window icon: $currentIconName. Following the theme uses: $themeIconName."
    }
}
